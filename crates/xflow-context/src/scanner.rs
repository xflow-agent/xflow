...
impl ProjectScanner {
    ...
    pub fn scan(&self) -> Result<ProjectInfo> {
        ...
        // 分离源代码文件
        let source_files: Vec<FileInfo> = files.iter().filter(|f| f.is_source).cloned().collect();

        // 排序语言统计
        let mut lang_vec: Vec<(_, _)> = language_stats
            .iter()
    ...
    fn detect_project_type(&self, files: &[FileInfo]) -> ProjectType {
        ...
        let has_package_json = files
            .iter()
            .any(|f| f.path.as_path() == Path::new("package.json"));
        ...
        let has_go_mod = files
            .iter()
            .any(|f| f.path.as_path() == Path::new("go.mod"));
        let has_pom_xml = files
            .iter()
            .any(|f| f.path.as_path() == Path::new("pom.xml"));
        ...
        let java_count = files
            .iter()
            .filter(|f| f.language == Language::Java || f.language == Language::Kotlin)
            .count();
